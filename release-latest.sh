...
asset_id=($(curl -u $GITHUB_ACTOR:$GH_TOKEN $CURL_RETRIES \
  ...
  https://api.github.com/repos/${GITHUB_REPOSITORY}/releases/tags/latest-llvm-clang-${{ env.BIT }} \
...
curl -u $GITHUB_ACTOR:$GH_TOKEN $CURL_RETRIES \
  ...
  -d '{"tag_name": "latest-llvm-clang-${{ env.BIT }}"}'
  
release_id=$(curl -u $GITHUB_ACTOR:$GH_TOKEN $CURL_RETRIES \
  -H "Accept: application/vnd.github.v3+json" \
  https://api.github.com/repos/${GITHUB_REPOSITORY}/releases/tags/latest-llvm-clang-${{ env.BIT }} | jq -r '.id')
